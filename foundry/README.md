...
2. Install the vault's own module from its deploy, `<vault>/_foundry/module.json`, and enable it. It requires this module and graft, so Foundry offers to install and enable both with it. The vault's module holds no code: a manifest, its packs, and one line naming the vault.
3. This module offers to build it. A role-gated vault asks once for a token (open the vault, sign in, paste it back); each reader gets only the pages their role may read.
