 ...
 */
export function compilePacks(jsonDir: string, outDir: string, moduleId: string): string[] {
  ...
  for (const pack of packs) {
    ...
    if (fs.existsSync(packOut)) fs.rmSync(packOut, { recursive: true });
    execFileSync(
      process.execPath,
      [FVTT_CLI, "package", "pack", "-n", pack, "--type", "Module", "--id", moduleId, "--in", inDir, "--out", outDir],
      { stdio: "pipe" },
    );
